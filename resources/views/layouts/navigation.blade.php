<aside class="h-full flex flex-col md:h-screen md:sticky md:top-0">
    ...
    <nav class="flex-1 px-4 py-6 space-y-2">
        ...
        @if(Auth::user()->role == 'admin_bk')
            ...
            <x-nav-link :href="route('admin.pengguna.index')" :active="request()->routeIs('admin.pengguna.*')">
                 <svg class="size-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372m-10.5-.372a9.369 9.369 0 0 1 2.625-.372M12 21a9.75 9.75 0 0 1-8.25-4.5m16.5 4.5a9.75 9.75 0 0 0-8.25-4.5m-8.25 4.5A9.75 9.75 0 0 1 12 3m0 18a9.75 9.75 0 0 0 8.25-4.5M3.75 4.5a9.75 9.75 0 0 1 8.25-4.5m0 18a9.75 9.75 0 0 1-8.25-4.5m16.5-4.5a9.75 9.75 0 0 0-8.25-4.5m-8.25 4.5A9.75 9.75 0 0 1 12 3m0 18a9.75 9.75 0 0 0-8.25-4.5M3.75 4.5a9.75 9.75 0 0 1 8.25-4.5" /></svg>
                {{ __('Pengguna Sistem') }}
            </x-nav-link>
            <x-nav-link :href="route('admin.jadwal.index')" :active="request()->routeIs('admin.jadwal.*')">
                <svg class="size-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
        ...
        @if(Auth::user()->role == 'guru_bk')
            <x-nav-link :href="route('guru.siswa.index')" :active="request()->routeIs('guru.siswa.*')">
             ...
             <x-nav-link :href="route('guru.pelanggaran-siswa.index')" :active="request()->routeIs('guru.pelanggaran-siswa.*')">
                <svg class="size-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75a3.375 3.375 0 0 1 6 0v1.5a3.375 3.375 0 0 1-6 0v-1.5a.375.375 0 0 0-.75 0V12m-9 3.75a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Z" /></svg>
    ...
    <div x-data="{ open: false }" class="px-4 py-4 border-t border-gray-200">
        ...
        <div x-show="open" x-cloak class="mt-2 space-y-1 bg-white rounded-lg shadow-inner text-sm text-gray-700">
            <a href="{{ route('profile.edit') }}" class="block px-4 py-2 hover:bg-gray-100 rounded-lg">{{ __('Profile') }}</a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full text-left px-4 py-2 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg">
                    {{ __('Log Out') }}
                </button>
            </form>
        </div>
